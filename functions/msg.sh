	...
	while read msg; do
		...
		while ((failed++ < retry)); do
			...
			if [ ! -z "$(echo "$data" | fgrep "消息发送成功")" ]; then
				echo -e "\e[93m$msg\e[0m" >&2
				break
			fi
			echo "$data" > "$debugfile"
			echo -e "\e[97m$(date '+%Y-%m-%d %H:%M:%S') \e[91m$msg: retry $failed...\e[0m" >&2
...
# Message handle And Print
function msgHandle_print
{
	msgcount=$(($(echo "$msgdata" | wc -l) / 3))
	#echo -e "\e[93mGot $msgcount messages.\e[0m"
	for ((i = 1; i < $msgcount + 1; i++)); do
		...
		sender=$(echo -n "$msg" | head -n 1 | sed "s/: \&nbsp\;\s*\$//")
		time=$(echo -n "$msg" | head -n 2 | tail -n 1 | sed "s/\([0-9][0-9]\:[0-9][0-9]\:[0-9][0-9]\).*\$/\1/")
		echo -e "\e[93mGot History Message:\e[0m \e[91m$sender\e[97m@\e[94m$time\e[97m: \e[92m$context\e[0m" | iconv -f UTF-8 -t "$encoding"
#		context=$(echo -e "$context")
	done
}
